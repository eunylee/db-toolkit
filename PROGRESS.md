...
- **미등록 단어 → 사전 등록 흐름**: 세그먼터가 "미등록"으로 표시한 구간마다 "사전에 추가" 버튼 → `AddTermModal`이 그 구간을 통짜로 등록하지 않고, 문자종류 경계(한글 덩어리/영문·숫자 덩어리, `core/dictionary/tokenize.py`)로 쪼갠 단어 후보별로 이미 있는지(`GET /dictionary/split-candidates`) 확인 → 이미 있으면 재사용, 없는 것만 각각 물리명+`DomainPickerModal`로 도메인 지정 후 `POST /dictionary/terms`(단건 upsert, source는 항상 custom으로 강제) → 전부 해결되면 "완료"로 그 컬럼만 `POST /naming/suggest` 재호출해 제안 갱신. **통짜 등록을 피하는 이유**: "외부URL"을 하나의 복합어로 등록하면 나중에 "관리자URL" 같은 다른 조합에 "URL"이 재사용 안 됨 — 단어 단위로 쌓여야 사전 매칭 방식의 가치가 생김.
...
- [x] Unit 3.5: 도메인 관리 + 미등록 단어 사전 등록 (F-103 보완, Unit 3 직후 진행)
  ...
  - **후속 개선(같은 세션 내)**: 미등록 구간을 통짜 복합어 하나로 등록하면 재사용이 안 됨을 사용자가 지적("외부URL" 등록 시 "외부"/"URL" 개별 존재 여부를 먼저 확인해야 함) → `core/dictionary/tokenize.py`(문자종류 경계로만 분리하는 `split_into_word_candidates`, 형태소분석기 아님) + `GET /dictionary/split-candidates`(후보별 존재여부 확인) 추가. AddTermModal을 후보별 행으로 재설계: 이미 있는 단어는 "이미 있음"으로 표시(재사용, 손 안 댐), 없는 단어만 각자 물리명+도메인 지정 후 등록, 전부 해결되면 "완료" 버튼으로 컬럼 제안 갱신. 백엔드 테스트 7개, 프런트 테스트 2개 추가(전체 102/45 passed). 실브라우저로 "외부"(기존 등록)+"ABC코드값"(신규) 조합 확인 — "ABC","코드값" 두 후보로 정확히 분리되어 각각 등록 후 물리명이 `EXT_ABC_CODE_VAL`로 정확히 조합됨.
...
- 2026-08-03: 미등록 단어 등록 방식 → 사용자가 "외부URL 등록 시 외부/URL 개별 존재 여부부터 확인하고 없는 것만 순서대로 추가돼야 한다"고 지적 → 통짜 복합어 등록에서 문자종류 경계 분리 + 후보별 존재확인/개별등록 방식으로 변경 (위 아키텍처 결정 참고).
